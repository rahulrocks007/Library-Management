...
<body>
    ...
    <div class="request-table">
        <div class="request-container book-container">
            <h2 class="request-title user-info-title">List of Books</h2>
        <?php
		if(isset($_POST['submit']))
		{
			$q = mysqli_query($db,"SELECT books.bookpic,books.bookid,books.bookname,authors.authorname,category.categoryname,books.ISBN,books.price,quantity from  `books`join `authors` on authors.authorid=books.authorid join `category` on category.categoryid=books.categoryid  where bookname like '%$_POST[search]%'; ");
		...
		{
			$res = mysqli_query($db,"SELECT books.bookpic,books.bookid,books.bookname,authors.authorname,category.categoryname,books.ISBN,books.price,quantity from  `books`join `authors` on authors.authorid=books.authorid join `category` on category.categoryid=books.categoryid ;");
            echo "<table class='rtable booktable'>";
            echo "<tr style='background-color: teal;'>";
            //Table header
            echo "<th>"; echo "Books"; echo "</th>";
